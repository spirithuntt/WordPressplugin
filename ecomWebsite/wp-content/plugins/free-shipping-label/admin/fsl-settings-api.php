...
class Settings_API
{
    ...
    {
        $html  = sprintf('<p class="info-description">%1$s</p>', $args['desc']);
    ...
    public function callback_text($args)
    {
        ...
        $html        = sprintf('<input type="%1$s" class="%2$s-text" id="%3$s[%4$s]" name="%3$s[%4$s]" value="%5$s" %6$s maxlength="%7$s"/>', $type, $size, $args['section'], $args['id'], $value, $placeholder, $maxlength);
    ...
    public function callback_number($args)
    {
        ...
        $has_unit_box = $args['unit'] ? 'has-unit-box' : '';

        $html        = sprintf('<input type="%1$s" class="%2$s-number %10$s" id="%3$s[%4$s]" name="%3$s[%4$s]" value="%5$s"%6$s%7$s%8$s%9$s/>', $type, $size, $args['section'], $args['id'], $value, $placeholder, $min, $max, $step, $has_unit_box);

        if ($has_unit_box) {
            $html .= '<div class="input-unit-box">' . $args['unit'] . '</div>';
        }
    ...
    public function callback_checkbox($args)
    {
        ...
        $html  .= '<span class="devnet-slider--round"></span>';
        $html  .= '</label>';
        $html  .= sprintf('<span class="description">%1$s</span>', $args['desc']);
        $html  .= '</fieldset>';
    ...
    public function callback_multicheck($args)
    {
        ...
        foreach ($args['options'] as $key => $label) {
            ...
            $html    .= sprintf('<label for="wpuf-%1$s[%2$s][%3$s]" class="switch">', $args['section'], $args['id'], $key);
            $html    .= sprintf('<input type="checkbox" class="checkbox" id="wpuf-%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked($checked, $key, false));
            $html  .= '<span class="devnet-slider--round"></span>';
            $html  .= '</label>';
            $html  .= sprintf('<span class="desc">%1$s</span><br>', $label);
        }
    ...
    {
        $value = $this->get_option($args['id'], $args['section'], $args['std']);
        $html  = '<fieldset class="radio-images">';

        foreach ($args['options'] as $key => $label) {
            $html .= sprintf('<label for="wpuf-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key);
            $html .= sprintf('<input type="radio" class="radio" id="wpuf-%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked($value, $key, false));
            $html .= sprintf('<span class="img">%1$s</span></label><br>', $label);
        }
    ...
    public function callback_select2($args)
    {
        ...
        $html  = sprintf('<select class="%1$s" name="%2$s[%3$s]%4$s" id="%2$s[%3$s]" %5$s>', $size, $args['section'], $args['id'], $multiple_collection, $multiple);
    ...
    public function callback_file($args)
    {
        ...
        $label = isset($args['options']['button_label']) ? $args['options']['button_label'] : esc_html__('Choose File', 'free-shipping-label');
